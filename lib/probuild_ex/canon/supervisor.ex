    ...
    canon_games =
      for platform_id <- games[:platform_ids] do
        Supervisor.child_spec(
          {
            ProbuildEx.Canon.Cron,
            {games[:delay], {ProbuildEx.Canon.Games, :run, [platform_id]}}
          },
    ...
    canon_pros =
      for platform_id <- pros[:platform_ids] do
        Supervisor.child_spec(
          {
            ProbuildEx.Canon.Cron,
            {pros[:delay], {ProbuildEx.Canon.Pros, :run, [platform_id]}}
          },
          id: String.to_atom("pro_" <> platform_id)
        )
      end

    Supervisor.init(canon_games ++ canon_pros, strategy: :one_for_one)
  end
end
